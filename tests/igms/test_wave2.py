...
def _visit(igm, keys, rng=None, **overrides):
    conn, repo = make_db()
    player = repo.create("Hero", "pw", "M")
    for key, value in overrides.items():
        setattr(player, key, value)
    gctx = make_ctx(conn, repo, player, keys=keys, rng=rng)
    return gctx, make_igm_ctx(gctx, igm), player, conn
...
async def test_salve_heals_for_gems():
    igm = Apothecary()
    _gctx, ctx, p, _ = _visit(igm, ["S", "x", "L"], gems=5, hp=4, hp_max=40)
    await igm.enter(ctx)
    assert p.gems == 3
    assert p.hp == 40
...
async def test_tonic_is_once_a_day_and_grants_a_fight():
    igm = Apothecary()
    gctx, ctx, p, _conn = _visit(igm, ["T", "x", "T", "x", "L"], gems=9, forest_fights=2)
    await igm.enter(ctx)
...
async def test_apothecary_refuses_when_you_cannot_pay():
    igm = Apothecary()
    _gctx, ctx, p, _ = _visit(igm, ["C", "x", "L"], gems=1)
    await igm.enter(ctx)
    assert p.gems == 1
    assert p.charm == 1
...
async def test_the_rate_is_shared_and_survives_the_visit():
    igm = GemTrader()
    _gctx, ctx, _p, _ = _visit(igm, ["L"], rng=SeqRandom([175]))
    await igm.enter(ctx)
    ctx.store.flush()
...
async def test_haggling_can_backfire():
    ...
    gctx, ctx, _p, _ = _visit(igm, ["H", "x", "L"], rng=SeqRandom([200]), charm=1)
    await igm.enter(ctx)
...
async def test_shallow_digging_is_safe():
    igm = AbandonedMines()
    _gctx, ctx, p, _ = _visit(igm, ["S", "x", "L"], rng=SeqRandom([75]), gold=0, hp=30)
    await igm.enter(ctx)
    assert p.gold == 75
    assert p.hp == 30
...
async def test_arena_refuses_a_bout_you_cannot_pay_for():
    igm = ArenaOfLords()
    gctx, ctx, p, _ = _visit(igm, ["1", "x", "L"], gold=10, level=5)
    await igm.enter(ctx)
    assert p.gold == 10
...
async def test_the_wall_remembers_what_you_wrote():
    igm = TheLatrine()
    gctx, ctx, _p, _ = _visit(igm, ["W", "Turgon smells", "x", "R", "x", "L"])
    await igm.enter(ctx)
    text = _screen(gctx)
