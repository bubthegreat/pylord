...
from tests.harness import play, query, query_one, screen
...
async def _two_players(attacker_overrides=None, target_overrides=None):
    database = await data.connect(":memory:")
    repo = database.players
    attacker = await repo.create("Hero", "pw", "M")
    for key, value in (attacker_overrides or {}).items():
        setattr(attacker, key, value)
    await repo.save(attacker)

    target = await repo.create("Victim", "pw", "F")
    for key, value in (target_overrides or {}).items():
        setattr(target, key, value)
    await repo.save(target)
    return database, repo, attacker, target


def _ctx(database, repo, player, keys):
...
async def test_list_excludes_self_online_dead_and_sleeping():
    database = await data.connect(":memory:")
    repo = database.players
    me = await repo.create("Hero", "pw", "M")
    online = await repo.create("Onliner", "pw", "M")
    online.online = 1
    await repo.save(online)
    dead = await repo.create("Deadman", "pw", "M")
    dead.alive = 0
    await repo.save(dead)
    sleeping = await repo.create("Sleeper", "pw", "M")
    sleeping.at_inn = 1
    await repo.save(sleeping)
    attackable = await repo.create("Attackable", "pw", "M")
    attackable.exp = 500
    await repo.save(attackable)

    ctx = _ctx(database, repo, me, ["x"])
...
async def test_slaughter_self_exclusion():
    ...
    repo = database.players
    me = await repo.create("Hero", "pw", "M")
    ctx = _ctx(database, repo, me, ["Hero", "y", "x"])
    await pvp_mod._slaughter(ctx)
    assert "wish to attack yourself" in screen(ctx.io)


async def test_slaughter_dead_target_refused():
    database, repo, attacker, _target = await _two_players(target_overrides={"alive": 0})
    ctx = _ctx(database, repo, attacker, ["Victim", "y", "x"])
...
async def test_slaughter_sleeping_target_refused():
    database, repo, attacker, _target = await _two_players(target_overrides={"at_inn": 1})
    ctx = _ctx(database, repo, attacker, ["Victim", "y", "x"])
    died = await pvp_mod._slaughter(ctx)
    assert died is False
...
async def test_slaughter_no_match_shows_message():
    database, repo, attacker, _target = await _two_players()
    ctx = _ctx(database, repo, attacker, ["Nobody Here", "x"])
    died = await pvp_mod._slaughter(ctx)
    assert died is False
...
async def test_slaughter_decline_confirm_does_not_fight():
    database, repo, attacker, _target = await _two_players()
    ctx = _ctx(database, repo, attacker, ["Victim", "y", "n"])
    died = await pvp_mod._slaughter(ctx)
    assert died is False
    assert (await query_one(database, "SELECT COUNT(*) c FROM mail")).c == 0
...
async def test_zero_player_fights_refuses_without_a_fight():
    database, repo, attacker, target = await _two_players(attacker_overrides={"player_fights": 0})
    ctx = _ctx(database, repo, attacker, ["x"])
...
async def test_online_target_blocked_and_costs_no_attempt():
    database, repo, attacker, target = await _two_players(target_overrides={"online": 1})
    ctx = _ctx(database, repo, attacker, ["x"])
    died = await pvp_mod.run_attack(ctx, target, from_inn=False)
    assert died is False
...
async def test_run_away_costs_a_fight_but_sends_no_mail_or_news():
    database, repo, attacker, target = await _two_players()
    ctx = _ctx(database, repo, attacker, ["r", "x"])
    ...
    assert (await query_one(database, "SELECT COUNT(*) c FROM mail")).c == 0
    assert (await query_one(database, "SELECT COUNT(*) c FROM daily_news")).c == 0
...
async def test_win_transfers_exact_gold_exp_gems_and_writes_news_and_mail():
    ...
    ctx = _ctx(database, repo, attacker, ["a", "x"])
    ...
    reloaded = await repo.get(target.id)
    ...
    news_row = await query_one(database, "SELECT text FROM daily_news")
...
async def test_loss_kills_attacker_and_credits_victim_directly():
    ...
    mail_row = await query_one(database, "SELECT text, effect FROM mail")
    assert "YOU HAVE BEEN ATTACKED" in mail_row.text
...
async def test_inn_sneak_attack_kills_sleeping_target_and_spends_bribe():
    ...
    ctx = _ctx(database, repo, attacker, keys)
    ...
    rows = await query(
        database, "SELECT text FROM mail WHERE to_id = :tid ORDER BY id", tid=target.id
    )
    assert len(rows) == 2
...
async def test_inn_bribe_declined_refunds_nothing_and_costs_nothing():
    database, repo, attacker, _target = await _two_players(attacker_overrides={"gold": 5000})
    ctx = _ctx(database, repo, attacker, ["n", "x"])
    died = await inn_mod._bribe_attack(ctx)
    assert died is False
...
async def test_inn_bribe_insufficient_gold_refused():
    database, repo, attacker, _target = await _two_players(attacker_overrides={"level": 5, "gold": 1})
    ctx = _ctx(database, repo, attacker, ["y", "x"])
    died = await inn_mod._bribe_attack(ctx)
    assert died is False
...
async def test_inn_bribe_level_gate_refuses_much_weaker_target():
    ...
    ctx = _ctx(database, repo, attacker, ["y", "s", "Victim", "y", "x", "r", "x"])
    died = await inn_mod._bribe_attack(ctx)
    assert died is False
...
async def test_inn_bribe_return_refunds_half():
    ...
    ctx = _ctx(database, repo, attacker, ["y", "r", "x"])
    died = await inn_mod._bribe_attack(ctx)
    assert died is False
...
async def test_win_counts_a_player_kill_and_persists_in_one_transaction():
    ...
    assert (await repo.get(target.id)).alive == 0
    assert (await query_one(database, "SELECT COUNT(*) c FROM mail")).c == 1
    assert (await query_one(database, "SELECT COUNT(*) c FROM daily_news")).c == 1
...
async def test_loss_never_persists_negative_hitpoints():
    ...
    assert (await repo.get(target.id)).pvp_kills == 1
