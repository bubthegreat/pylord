...
async def test_deposit_and_withdraw_roundtrip():
    ctx = await _ctx(overrides={"gold": 500, "bank": 0}, keys=["d", "300", "w", "300", "r"])
...
async def test_deposit_all_shortcut():
    ctx = await _ctx(overrides={"gold": 500, "bank": 0}, keys=["d", "1", "r"])
...
async def test_withdraw_all_shortcut():
    ctx = await _ctx(overrides={"gold": 0, "bank": 500}, keys=["w", "1", "r"])
    await bank_mod.bank(ctx)
    assert ctx.player.gold == 500
    assert ctx.player.bank == 0
...
async def test_cannot_withdraw_more_than_in_bank():
    ctx = await _ctx(overrides={"gold": 0, "bank": 100}, keys=["w", "200", "r"])
    await bank_mod.bank(ctx)
    text = screen(ctx.io)
...
async def test_cannot_deposit_more_than_on_hand():
    ctx = await _ctx(overrides={"gold": 100, "bank": 0}, keys=["d", "200", "r"])
    await bank_mod.bank(ctx)
    text = screen(ctx.io)
